...
# Combining Angles, Mutations, and Distance Data
...
def load_and_combine_data():
    """
    Load and combine all three data types: angles, mutations, and distances
    """
    print("=" * 70)
    ...
    # Process mutation data (remove sequence and variant columns for features)
    mutation_features = mutation_df.drop(columns=['sequence', 'variant'])
    mutation_features = mutation_features.astype(float)
    
    print(f"   Mutation data shape: {mutation_df.shape}")
    print(f"   Mutation features: {len(mutation_features.columns)}")
    
    # 3. Load distance data
    print("   Loading distance data...")
    try:
        distance_df = pd.read_csv("tmalign/residues_full.csv")
        distance_features = distance_df.drop(columns=['sequence'])
        distance_features = distance_features.astype(float)
        print(f"   Distance data shape: {distance_df.shape}")
        print(f"   Distance features: {len(distance_features.columns)}")
    ...
    return combined_df, angle_cols

def extract_features_for_clustering(combined_df, angle_cols):
    """
    ...
    # 2. Mutation features (exclude sequence, variant, and 'Against' columns)
    ...
    # 3. Distance features (if available)
    ...
    return X, feature_names, feature_groups, combined_df

def create_comprehensive_dendrogram(X, feature_names, combined_df, feature_groups):
    """
    Create comprehensive dendrogram analysis with multiple linkage methods
    """
    ...
    # Create a detailed dendrogram with the best method (Ward)
    ...
    return linkage_matrix, X_scaled

def analyze_cluster_structure(linkage_matrix, combined_df, X_scaled):
    """
    Analyze the cluster structure and provide insights
    """
    ...
    # Analyze cluster distribution for different numbers of clusters
...
def print_analysis_summary(combined_df, feature_groups, linkage_matrix):
    """
...
def main():
    """
    Main function to run the comprehensive dendrogram analysis
    """
    # Load and combine data
    combined_df, angle_cols = load_and_combine_data()
    
    # Extract features
    X, feature_names, feature_groups, combined_df = extract_features_for_clustering(combined_df, angle_cols)
    
    # Create comprehensive dendrogram
    linkage_matrix, X_scaled = create_comprehensive_dendrogram(X, feature_names, combined_df, feature_groups)
    
    # Analyze cluster structure
    cluster_labels = analyze_cluster_structure(linkage_matrix, combined_df, X_scaled)
    
    # Print summary
    print_analysis_summary(combined_df, feature_groups, linkage_matrix)
